...
// https://github.com/ldeffenb/Swarming-Wikipedia/blob/main/src/index.ts
...
import { NodeType } from "mantaray-js";
  ...
  await (async () => {
    for await (const f of getFiles(sourcePath)) {
      ...
      if (posixPath.slice(0, 2) == "A/") {
        // These are the HTML documents in a zim archive
        if (index && posixPath == index) hasIndex = true;
        else {
          const linkPath = posixPath
            .replace(/%/g, "%25")
            .replace(/\?/g, "%3F")
            .replace(/&/g, "%26")
            .replace(/\"/g, "%22");
          const visiblePath = posixPath
            .slice(2)
            .replace(/</g, "&lt;")
            .replace(/>/g, "&gt;")
            .replace(/_/g, " ");
          indexHTML =
            indexHTML + `<LI><A HREF="${linkPath}">${visiblePath}</A></LI>\n`;
        }
      }
      fileCount++;
      const heap = process.memoryUsage();
...
async function processAndUpload(srcDir: string) {
  ...
  return { hash: rootNode, tagId: tagID };
}

export { processAndUpload, setBatchID };
